...
def calculate_mixture_ate(
    ...
    for component in mixture.components:
        ...
        routes_map = {
            ...
            "inhalation_dusts_mists": (
                "dust_mist",
                substance.ate_inhalation_dusts_mists,
            ),
        }
...
def _determine_ate_category(ate_value: float, route: str) -> int:
    ...
    if "oral" in route:
        norm_route = "oral"
    elif "dermal" in route:
        norm_route = "dermal"
    elif "gas" in route:
        norm_route = "gas"
    elif "vapour" in route:
        norm_route = "vapour"
    elif "dust" in route or "mist" in route:
        norm_route = "dust_mist"
...
def classify_by_atemix(
    atemix_results: Dict[str, Optional[float]],
    ...
    for route, atemix_value in atemix_results.items():
        ...
        if category < 5:
            ...
            if hazard_info:
                ...
                h_code = None
                if route == "oral":
                    h_code = f"H30{h_code_suffix}"
                elif route == "dermal":
                    h_code = f"H31{h_code_suffix}"
                elif route.startswith("inhalation"):
                    h_code = f"H33{h_code_suffix}"
        ...
        log_entries.append(
            {"step": f"Klasifikace ATE - {route_cz}", "detail": log_msg, "result": "OK"}
        )
