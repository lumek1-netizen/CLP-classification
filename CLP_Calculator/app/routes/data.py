        ...
        for m_data in data.get('mixtures', []):
            if not Mixture.query.filter_by(name=m_data['name']).first():
                ...
                for c_data in m_data.get('components', []):
                    sub = Substance.query.filter_by(name=c_data['substance_name']).first()
                    if sub:
                        db.session.add(MixtureComponent(mixture_id=new_mix.id, substance_id=sub.id, concentration=c_data['concentration']))
